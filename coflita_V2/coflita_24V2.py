...
def main(page: ft.Page):
    ...
    def navigate(e, view, user_name=None):
        ...
        nonlocal current_user, is_logged_in
        ...
        # si voy al logout, cierro sesión y voy al home
        if view == "logout":
            current_user = None
            is_logged_in = False
            update_nav_bar()
        ...
        # si el login es exitoso, actualizar el estado del usuario
        if view == "login_success":
            current_user = user_name
            is_logged_in = True
            # Restaurar el sistema normal de navegación
            page.controls.clear()
        ...
        # si el registro es exitoso, actualizar el estado del usuario
        ...
        # 👉 si voy al home (desde login/register o cualquier otra vista)
        if view == "home":
            # Si estamos en una vista de login/register, restaurar el sistema normal
            if len(page.controls) == 1 and hasattr(page.controls[0], 'content'):
                page.controls.clear()
                page.add(content_container)
            current_view = "home"
            update_view()
    ...
    def create_nav_bar_content_row():
        ...
        # Agregar botones de usuario dependiendo del estado de login
            ...
            nav_items.extend([
                ft.Container(
                    content=ft.Row([
                        ...
                        ft.Text(f"👤 {current_user}", color=COLOR_WHITE, weight=ft.FontWeight.BOLD)
                    ]),
                    padding=ft.padding.symmetric(horizontal=10)
                ),
                ft.ElevatedButton("Cerrar Sesión", on_click=lambda e: navigate(e, "logout"),
                                style=ft.ButtonStyle(bgcolor=COLOR_AMBER_700, color=COLOR_WHITE))
            ])
        else:
            # Usuario no logueado: mostrar botones de login y registro
            nav_items.extend([
                ft.ElevatedButton("Iniciar Sesión", on_click=lambda e: navigate(e, "login"),
